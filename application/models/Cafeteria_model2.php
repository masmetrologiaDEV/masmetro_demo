...
class Cafeteria_model extends CI_Model {
    ...
    public function verTicket($idTicket) {
        $this->db->select('TS.id, TS.fecha, ifnull(concat(U.nombre," ",U.paterno),"N/A") as User, U.correo, ifnull((SELECT concat(nombre," ",paterno) from usuarios where id = TS.cierre), "0") as Cierre, TS.fecha_cierre, TS.tipo, TS.titulo, TS.descripcion, TS.estatus, TS.id_user');
